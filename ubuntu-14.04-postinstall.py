...
_CONF_FILE = "ubuntu-14.04-unity-postinstall.cfg"
...
def showexec(description, command, exitonerror=0, presskey=0, waitmessage=""):
    ...
    sys.stdout.write(Colors.NO + "%s" % waitmessage + statuscolor +
                     "%s" % status.rjust(79-len(waitmessage)) + Colors.NO)
    sys.stdout.flush()
    ...
    sys.stdout.write(Colors.NO + "\r%s" % description + statuscolor +
...
def getpassword(description=""):
    ...
    if password1 == password2:
        ...
        sys.stdout.write(Colors.ORANGE +
                         _("[Warning] Password did not match, please try "
                           "again") + Colors.NO + "\n")
        return getpassword()
...
def main(argv):
    ...
    if _UBUNTU_VERSION != _FOR_UBUNTU:
        showexec(_("Script only for Ubuntu %s") % _FOR_UBUNTU,
                 "badubuntuversion", exitonerror=1)

    # Read the configuration file
    if config_file == "":
        config_file = "/tmp/%s.cfg" % __appname__
        showexec(_("Download the configuration file"),
                 "rm -f "+config_file+" ; "+_WGET+
                 " -O "+config_file+" "+config_url)
    config = ConfigParser.RawConfigParser()
    config.read(config_file)

    if config.has_section("gnome3") and config.has_section("unity"):
        showexec(_("Can not use both Gnome 3 and Unity,"
                   " please change your .cfg file"),
                 "gnome3etunitygrosboulet", exitonerror=1)

    # Parse and exec pre-actions
    for action_name, action_cmd in config.items("preactions"):
        showexec(_("Execute preaction ")+action_name.lstrip("action_"),
                 action_cmd)
    ...
    for item_type, item_value in config.items("repos"):
        if item_type.startswith("ppa_"):
            showexec(_("Install repository ")+item_type.lstrip("ppa_"),
                     _APT_ADD+" "+item_value)
        elif item_type.startswith("url_"):
            showexec(_("Install repository ")+item_type.lstrip("url_"),
                     _APT_ADD+" \\\"deb "+item_value+"\\\"")
        elif item_type.startswith("key_"):
            showexec(_("Install key for the ")+item_type.lstrip("key_")+
                     " repository", _APT_KEY+" "+item_value)
    ...
    for pkg_type, pkg_list in config.items("packages"):
        if pkg_type.startswith("remove_"):
            showexec(_("Remove packages ")+pkg_type.lstrip("remove_"),
    ...
    for pkg in pkg_list_others.keys():
        showexec(_("Install packages ")+pkg,
                 _APT_INSTALL+" "+pkg_list_others[pkg])

    # Allow user to read DVD (CSS)
    showexec(_("DVDs CSS encryption reader"),
    ...
    if config.has_section("dotfiles"):
        ...
        showexec(_("Create the ~/.bashrc.d subfolder"),
                 "mkdir -p $HOME/.bashrc.d")
        if config.has_option("dotfiles", "bashrc"):
            showexec(_("Download bash main configuration file"),
                     _WGET+" -O $HOME/.bashrc "+config.get("dotfiles", "bashrc"))
        if config.has_option("dotfiles", "bashrc_prompt"):
            showexec(_("Download bash prompt configuration file"),
                     _WGET+" -O $HOME/.bashrc.d/bashrc_prompt "+
                     config.get("dotfiles", "bashrc_prompt"))
        if config.has_option("dotfiles", "bashrc_aliases"):
            showexec(_("Download bash aliases configuration file"),
                     _WGET+" -O $HOME/.bashrc.d/bashrc_aliases "+
                     config.get("dotfiles", "bashrc_aliases"))
        showexec(_("Install the bash configuration file"),
                 "chown -R $USERNAME:$USERNAME $HOME/.bashrc*")
        # Vim
        if config.has_option("dotfiles", "vimrc"):
            showexec(_("Donwload the Vim configuration file"),
                     _WGET+" -O $HOME/.vimrc "+config.get("dotfiles", "vimrc"))
            showexec(_("Install the Vim configuration file"),
                     "chown -R $USERNAME:$USERNAME $HOME/.vimrc")

        # Htop
        if config.has_option("dotfiles", "htoprc"):
            showexec(_("Download the Htop configuration file"),
                     _WGET+" -O $HOME/.htoprc "+config.get("dotfiles", "htoprc"))
            showexec(_("Install the Htop configuration file"),
                     "chown -R $USERNAME:$USERNAME $HOME/.htoprc")

        # Pythonrc
        if config.has_option("dotfiles", "pythonrc"):
            showexec(_("Download the Pythonrc configuration file"),
                     _WGET+" -O $HOME/.pythonrc "+
                     config.get("dotfiles", "pythonrc"))
            showexec(_("Install the Pythonrc configuration file"),
                     "chown -R $USERNAME:$USERNAME $HOME/.pythonrc")

    # Gnome 3 configuration
    if config.has_section("gnome3"):
        # Set the default theme
        if config.has_option("gnome3", "theme"):
            showexec(_("Set the default Gnome Shell theme to ")+
                     config.get("gnome3", "theme"),
                     "sudo -u $USERNAME gsettings set org.gnome.desktop.interface gtk-theme "+
                     config.get("gnome3", "theme"))
        # Set the default icons
        if config.has_option("gnome3", "icons"):
            showexec(_("Set the default Gnome Shell icons to ")+
                     config.get("gnome3", "icons"),
                     "sudo -u $USERNAME gsettings set org.gnome.desktop.interface icon-theme "+
                     config.get("gnome3", "icons"))
        # Set the default cursors
        if config.has_option("gnome3", "cursors"):
            showexec(_("Set the default Gnome Shell cursors to ")+
                     config.get("gnome3", "cursors"),
                     "sudo -u $USERNAME gsettings set org.gnome.desktop.interface cursor-theme "+
                     config.get("gnome3", "cursors"))
        # Download and install the default Conky configuration
        if config.has_option("gnome3", "conky"):
            showexec(_("Download the Conky configuration file"),
                     _WGET+" -O $HOME/.conkyrc "+config.get("gnome3", "conky"))
            showexec(_("Install the Conky configuration file"),
                     "chown -R $USERNAME:$USERNAME $HOME/.conkyrc")
        # Get the minimize/maximize button and ALT-F2 shortcut back
        showexec(_("Get the minimize and maximize button back in Gnome Shell"),
                 "sudo -u $USERNAME gconftool-2 -s -t string /desktop/gnome/shell/windows/button_layout \":minimize,maximize,close\"")
        showexec(_("Get ALT-F2 back to me"),
                 "sudo -u $USERNAME gconftool-2 --recursive-unset /apps/metacity/global_keybindings")
        # Gnome Shell is the default UI
        showexec(_("Gnome Shell is now the default shell"),
    ...
    if config.has_section("unity"):
        ...
        if config.has_option("unity", "theme"):
            showexec(_("Set the default Unity theme to ")+
                     config.get("unity", "theme"),
                     "gsettings set org.gnome.desktop.interface gtk-theme "+
                     config.get("unity", "theme"))
        # Set the default icons
        if config.has_option("unity", "icons"):
            showexec(_("Set the default Unity icons to ")+
                     config.get("unity", "icons"),
                     "gsettings set org.gnome.desktop.interface icon-theme "+
                     config.get("unity", "icons"))
        # Set the default cursors
        if config.has_option("unity", "cursors"):
            showexec(_("Set the default Unity cursors to ")+
                     config.get("unity", "cursors"),
                     "gsettings set org.gnome.desktop.interface cursor-theme "+
                     config.get("unity", "cursors"))
        # Download and install the default Conky configuration
        if config.has_option("unity", "conky"):
            showexec(_("Install the Conky configuration file"),
                     _WGET+" -O $HOME/.conkyrc "+config.get("unity", "conky"))
        # Unity is the default UI
        showexec(_("Unity is now the default shell"),
                 "/usr/lib/lightdm/lightdm-set-defaults -s unity-3d")

    # Parse and exec post-actions
    for action_name, action_cmd in config.items("postactions"):
        showexec(_("Execute postaction ")+
                 action_name.lstrip("action_"), action_cmd)
